    ...
    <div className="group bg-[#141414] border border-[#2a2a2a] hover:border-[#C9A84C]/30 rounded-2xl p-6 transition-all duration-300 hover:bg-[#1a1a1a] flex flex-col">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-[#C9A84C] text-sm font-medium">{tagline}</p>
        </div>
      </div>

      <p className="text-[#555555] text-sm leading-relaxed mb-5 flex-1">
        {description}
      </p>

      <div className="flex flex-wrap gap-1.5 mb-5">
        {stack.map(s => (
          <span key={s} className="bg-white/[0.04] border border-white/8 text-[#888888] text-xs px-2.5 py-1 rounded-lg font-mono">
            {s}
          </span>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-[#2a2a2a]">
        ...
        <div className="flex items-center gap-3">
          {github !== "#" && (
            <a href={github} target="_blank" rel="noreferrer" className="text-[#555555] hover:text-[#C9A84C] transition-colors">
              <GitBranch size={16} />
            </a>
          )}
          {live !== "#" && (
            <a href={live} target="_blank" rel="noreferrer" className="text-[#555555] hover:text-[#C9A84C] transition-colors">
